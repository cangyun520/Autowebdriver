...
class HTContract(unittest.TestCase):
    def setUp(self):
    ...
    def test_0520_01(self):
        ...
        for i in v_tip:
            if "发布成功" in i.text:
                ...
                print(i.text)
            elif "还不存在该员工" in i.text:
                print(i.text)
            elif "存在该员工的新签合同" in i.text:
                print(i.text)
            else:
                driver.get_screenshot_as_file(root_path() + 'TestPicture/hr/test_0520_01.jpg')
                print("Error：" + i.text)
                unittest.expectedFailure("test_0520_01")

    """人事管理-员工合同-合同管理数据查看"""
    def test_0520_02_look(self):
        """人事管理-员工合同-合同管理数据查看"""
        driver = self.driver
        v_list = driver.find_elements_by_class_name("x-grid3-row")
        if len(v_list) > 0:
            ...
            v_span = driver.find_elements_by_class_name("x-tab-strip-text")
            for i in v_span:
                if "工作流程" in i.text:
                    print("合同管理数据查看正常")
                    break
                else:
                    driver.get_screenshot_as_file(root_path() + 'TestPicture/hr/test_0520_02_look.jpg')
                    print(i.text)
                    unittest.expectedFailure("test_0520_02_look")
        else:
            print("列表数据为空，无法查看")
...
if __name__ == "__main__":
    ...
    testsuit.addTest(HTContract("test_0520_01"))
    # 执行测试集合
